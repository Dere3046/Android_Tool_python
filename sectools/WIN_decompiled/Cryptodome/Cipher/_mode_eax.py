...
class EaxMode(object):
    ...
    def __init__(self, factory, key, nonce, mac_len, cipher_params):
        ...
        self._next = [
            ...
            self.digest,
            self.verify]
        if not self._mac_len <= self._mac_len or self._mac_len <= self.block_size:
            pass
        else:
            4
        ...
        self._omac = (lambda .0 = None: [ CMAC.new(key, b'\x00' * (self.block_size - 1) + struct.pack('B', i), factory, cipher_params, **('ciphermod', 'cipher_params')) for i in .0 ])(range(0, 3))
    ...
    def verify(self, received_mac_tag):
        ...
        mac1 = BLAKE2s.new(160, secret, self._mac_tag, **('digest_bits', 'key', 'data'))
        mac2 = BLAKE2s.new(160, secret, received_mac_tag, **('digest_bits', 'key', 'data'))
