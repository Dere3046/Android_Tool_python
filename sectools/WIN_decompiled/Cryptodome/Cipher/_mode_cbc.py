...
class CbcMode(object):
    ...
    def __init__(self, block_cipher, iv):
        ...
        self._state = None(self._state.get(), raw_cbc_lib.CBC_stop_operation)
    ...
    def encrypt(self, plaintext, output = (None,)):
        ...
        if output is None:
            ...
            result = None.CBC_encrypt(self._state.get(), c_uint8_ptr(plaintext), c_uint8_ptr(ciphertext), c_size_t(len(plaintext)))
            if result:
                if result == 3:
                    raise ValueError('Data must be padded to %d byte boundary in CBC mode' % self.block_size)
                raise None('Error %d while encrypting in CBC mode' % result)
            if None is None:
    ...
    def decrypt(self, ciphertext, output = (None,)):
        ...
        if output is None:
            ...
            if None(ciphertext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.CBC_decrypt(self._state.get(), c_uint8_ptr(ciphertext), c_uint8_ptr(plaintext), c_size_t(len(ciphertext)))
            if result:
                if result == 3:
                    raise ValueError('Data must be padded to %d byte boundary in CBC mode' % self.block_size)
                raise None('Error %d while decrypting in CBC mode' % result)
            if None is None:
