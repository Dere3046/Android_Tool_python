...
class CfbMode(object):
    ...
    def __init__(self, block_cipher, iv, segment_size):
        ...
        self._state = None(self._state.get(), raw_cfb_lib.CFB_stop_operation)
    ...
    def encrypt(self, plaintext, output = (None,)):
        ...
        if output is None:
            ...
            result = None.CFB_encrypt(self._state.get(), c_uint8_ptr(plaintext), c_uint8_ptr(ciphertext), c_size_t(len(plaintext)))
            if result:
    ...
    def decrypt(self, ciphertext, output = (None,)):
        ...
        if output is None:
            ...
            if None(ciphertext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.CFB_decrypt(self._state.get(), c_uint8_ptr(ciphertext), c_uint8_ptr(plaintext), c_size_t(len(ciphertext)))
            if result:
...
def _create_cfb_cipher(factory, **kwargs):
    ...
    (segment_size_bytes, rem) = None(kwargs.pop('segment_size', 8), 8)
    ...
    return None(cipher_state, iv, segment_size_bytes)
