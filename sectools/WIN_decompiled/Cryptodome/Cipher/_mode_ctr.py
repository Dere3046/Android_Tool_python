...
class CtrMode(object):
    ...
    def __init__(self, block_cipher, initial_counter_block, prefix_len, counter_len, little_endian):
        ...
        self._state = None(self._state.get(), raw_ctr_lib.CTR_stop_operation)
    ...
    def encrypt(self, plaintext, output = (None,)):
        ...
        if output is None:
            ...
            if None(plaintext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.CTR_encrypt(self._state.get(), c_uint8_ptr(plaintext), c_uint8_ptr(ciphertext), c_size_t(len(plaintext)))
            if result:
                if result == 393218:
                    raise OverflowError('The counter has wrapped around in CTR mode')
                raise None('Error %X while encrypting in CTR mode' % result)
            if None is None:
    ...
    def decrypt(self, ciphertext, output = (None,)):
        ...
        if output is None:
            ...
            if not is_writeable_buffer(output):
                raise TypeError('output must be a bytearray or a writeable memoryview')
            if None(ciphertext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.CTR_decrypt(self._state.get(), c_uint8_ptr(ciphertext), c_uint8_ptr(plaintext), c_size_t(len(ciphertext)))
            if result:
                if result == 393218:
                    raise OverflowError('The counter has wrapped around in CTR mode')
                raise None('Error %X while decrypting in CTR mode' % result)
            if None is None:
...
def _create_ctr_cipher(factory, **kwargs):
    ...
    if None is not None and (nonce, initial_value) != (None, None):
    ...
    if None is None:
        if nonce is None:
            ...
            nonce = None(factory.block_size // 2)
        ...
        if is_native_int(initial_value):
            ...
            initial_counter_block = None + long_to_bytes(initial_value, counter_len)
        ...
        return CtrMode(cipher_state, initial_counter_block, len(nonce), counter_len, False)
    _counter = None(counter)
