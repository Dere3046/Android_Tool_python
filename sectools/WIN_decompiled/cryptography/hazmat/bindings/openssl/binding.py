...
class Binding:
    ...
    _ensure_ffi_initialized = None(_ensure_ffi_initialized)
    
    def init_static_locks(cls = None):
        cls._ensure_ffi_initialized()

    init_static_locks = None(init_static_locks)
...
def _verify_package_version(version = None):
    ...
    None(_openssl.lib.OpenSSL_version_num() == openssl.openssl_version())

_verify_package_version(cryptography.__version__)
Binding.init_static_locks()
if sys.platform == 'win32' or os.environ.get('PROCESSOR_ARCHITEW6432') is not None:
    warnings.warn('You are using cryptography on a 32-bit Python on a 64-bit Windows Operating System. Cryptography will be significantly faster if you switch to using a 64-bit Python.', UserWarning, 2, **('stacklevel',))
    return None
return None
