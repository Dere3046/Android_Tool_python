...
X25519PublicKey = <NODE:27>((lambda : 
def from_public_bytes(cls = None, data = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.x25519_supported():
raise UnsupportedAlgorithm('X25519 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_EXCHANGE_ALGORITHM)None.x25519.from_public_bytes(data)from_public_bytes = None(from_public_bytes)
...
'''
        Checks equality.
        '''
pass__eq__ = None(__eq__)), 'X25519PublicKey', abc.ABCMeta, **('metaclass',))
X25519PublicKey.register(rust_openssl.x25519.X25519PublicKey)
X25519PrivateKey = <NODE:27>((lambda : 
def generate(cls = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.x25519_supported():
raise UnsupportedAlgorithm('X25519 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_EXCHANGE_ALGORITHM)None.x25519.generate_key()generate = None(generate)
def from_private_bytes(cls = None, data = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.x25519_supported():
raise UnsupportedAlgorithm('X25519 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_EXCHANGE_ALGORITHM)None.x25519.from_private_bytes(data)from_private_bytes = None(from_private_bytes)
...
"""
        ...
        """
passexchange = None(exchange)), 'X25519PrivateKey', abc.ABCMeta, **('metaclass',))
