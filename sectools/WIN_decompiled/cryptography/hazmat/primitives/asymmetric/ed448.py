...
Ed448PublicKey = <NODE:27>((lambda : 
def from_public_bytes(cls = None, data = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.ed448_supported():
raise UnsupportedAlgorithm('ed448 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)None.ed448.from_public_bytes(data)from_public_bytes = None(from_public_bytes)
...
'''
        Checks equality.
        '''
pass__eq__ = None(__eq__)), 'Ed448PublicKey', abc.ABCMeta, **('metaclass',))
if hasattr(rust_openssl, 'ed448'):
    Ed448PublicKey.register(rust_openssl.ed448.Ed448PublicKey)
Ed448PrivateKey = <NODE:27>((lambda : 
def generate(cls = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.ed448_supported():
raise UnsupportedAlgorithm('ed448 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)None.ed448.generate_key()generate = None(generate)
def from_private_bytes(cls = None, data = None):
backend = backendimport cryptography.hazmat.backends.openssl.backendif not backend.ed448_supported():
raise UnsupportedAlgorithm('ed448 is not supported by this version of OpenSSL.', _Reasons.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)None.ed448.from_private_bytes(data)from_private_bytes = None(from_private_bytes)
...
'''
        ...
        '''
passprivate_bytes_raw = None(private_bytes_raw)), 'Ed448PrivateKey', abc.ABCMeta, **('metaclass',))
