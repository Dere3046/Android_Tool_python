...
class InitVar:
    ...
    def __repr__(self):
        if not isinstance(self.type, type) and isinstance(self.type, GenericAlias):
...
def _fields_in_init_order(fields):
    return (tuple((lambda .0: for f in .0:
if not f.init and f.kw_only:
fcontinueNone)(fields)), tuple((lambda .0: for f in .0:
if f.init and f.kw_only:
...
def _init_fn(fields, std_fields, kw_only_fields, frozen, has_post_init, self_name, globals, slots):
    ...
    for f in std_fields:
        if f.init:
            ...
            if has_post_init:
                params_str = ','.join((lambda .0: for f in .0:
...
def _frozen_get_del_attr(cls, fields, globals):
    ...
    if fields:
        fields_str = '(' + ','.join((lambda .0: for f in .0:
repr(f.name))(fields)) + ',)'
    else:
        fields_str = '()'
