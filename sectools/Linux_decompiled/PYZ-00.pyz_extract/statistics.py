...
def harmonic_mean(data, weights = (None,)):
    ...
    (_, sum_weights, _) = None((lambda .0: for w in .0:
w)(_fail_neg(weights, errmsg)))
# WARNING: Decompyle incomplete
...
def _ss(data, c = (None,)):
    ...
    '''
    if c is not None:
        (T, total, count) = None((lambda .0 = None: for x in .0:
(x - c) ** 2)(data))
        return (T, total)
    (T, total, count) = None(data)
...
def covariance(x, y):
    ...
    xbar = None(x) / n
    ybar = fsum(y) / n
    sxy = None((lambda .0 = None: for xi, yi in .0:
(xi - xbar) * (yi - ybar))(zip(x, y)))
    return sxy / (n - 1)
...
def correlation(x, y):
    ...
    sxy = None((lambda .0 = None: for xi, yi in .0:
(xi - xbar) * (yi - ybar))(zip(x, y)))
    sxx = None((lambda .0 = None: for xi in .0:
(xi - xbar) ** 2)(x))
    syy = None((lambda .0 = None: for yi in .0:
(yi - ybar) ** 2)(y))
# WARNING: Decompyle incomplete
