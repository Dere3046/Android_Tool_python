...
class _GzipReader(_compression.DecompressReader):
    ...
    def _read_eof(self):
        ...
        if None != self._stream_size & 0xFFFFFFFFL:
...
def main():
    ...
    for arg in args.args:
        if args.decompress:
            if arg == '-':
                f = GzipFile('', 'rb', sys.stdin.buffer, **('filename', 'mode', 'fileobj'))
                g = sys.stdout.buffer
            elif arg[-3:] != '.gz':
            ...
            f = sys.stdin.buffer
            g = GzipFile('', 'wb', sys.stdout.buffer, compresslevel, **('filename', 'mode', 'fileobj', 'compresslevel'))
        else:
