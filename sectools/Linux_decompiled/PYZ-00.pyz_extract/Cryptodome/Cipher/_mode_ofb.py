...
class OfbMode(object):
    ...
    def __init__(self, block_cipher, iv):
        ...
        self._state = None(self._state.get(), raw_ofb_lib.OFB_stop_operation)
    ...
    def encrypt(self, plaintext, output = (None,)):
        ...
        if output is None:
            ...
            result = None.OFB_encrypt(self._state.get(), c_uint8_ptr(plaintext), c_uint8_ptr(ciphertext), c_size_t(len(plaintext)))
            if result:
    ...
    def decrypt(self, ciphertext, output = (None,)):
        ...
        if output is None:
            ...
            if None(ciphertext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.OFB_decrypt(self._state.get(), c_uint8_ptr(ciphertext), c_uint8_ptr(plaintext), c_size_t(len(ciphertext)))
            if result:
