...
def make_header(decoded_seq, maxlinelen, header_name, continuation_ws = (None, None, ' ')):
    ...
    for s, charset in decoded_seq:
        if not charset is not None and isinstance(charset, Charset):
        ...
        for string, charset in self._chunks:
            ...
            if uchunks:
                ...
                if lastcs not in (None, 'us-ascii'):
                    if not nextcs in (None, 'us-ascii') and hasspace:
    ...
    def encode(self, splitchars, maxlinelen, linesep = (';, \t', None, '\n')):
        ...
        for string, charset in self._chunks:
            if hasspace is not None:
                ...
                hasspace = self._nonctext(string[0])
                if lastcs not in (None, 'us-ascii'):
                    if hasspace or charset not in (None, 'us-ascii'):
                        formatter.add_transition()
                    elif not charset not in (None, 'us-ascii') and lastspace:
                        formatter.add_transition()
            if string:
                pass
            lastspace = self._nonctext(string[-1])
...
class _Accumulator(list):
    ...
    def __len__(self):
        return sum((lambda .0: for fws, part in .0:
len(fws) + len(part))(self), self._initial_size)

    
    def __str__(self):
        return EMPTYSTRING.join((lambda .0: for fws, part in .0:
EMPTYSTRING.join((fws, part)))(self))
