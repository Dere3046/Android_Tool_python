...
class Tag:
    ...
    def __repr__(self = None):
        return '<{self} @ {self_id}>'.format(self, id(self), **('self', 'self_id'))
...
def _py_interpreter_range(py_version = None):
    ...
    '''
    if len(py_version) > 1:
        yield 'py{version}'.format(_version_nodot(py_version[:2]), **('version',))
    yield 'py{major}'.format(py_version[0], **('major',))
    if len(py_version) > 1:
        for minor in range(py_version[1] - 1, -1, -1):
            yield 'py{version}'.format(_version_nodot((py_version[0], minor)), **('version',))
    return None
...
def _mac_binary_formats(version = None, cpu_arch = None):
    ...
    if cpu_arch == 'x86_64':
        ...
        None.extend([
            'fat32',
            'fat'])
    if cpu_arch in frozenset({'x86_64', 'arm64'}):
        formats.append('universal2')
    if cpu_arch in frozenset({'x86_64', 'ppc64', 'i386', 'intel', 'ppc'}):
