...
class _Traces(Sequence):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            return tuple((lambda .0: for trace in .0:
Trace(trace))(self._traces[index]))
...
class Filter(BaseFilter):
    ...
    def _match_traceback(self, traceback):
        if self.all_frames:
            if None((lambda .0 = None: for filename, lineno in .0:
self._match_frame_impl(filename, lineno))(traceback)):
...
class Snapshot:
    ...
    def _filter_trace(self, include_filters, exclude_filters, trace):
        if not include_filters and None((lambda .0 = None: for trace_filter in .0:
trace_filter._match(trace))(include_filters)):
            return False
        if None and None((lambda .0 = None: for trace_filter in .0:
not trace_filter._match(trace))(exclude_filters)):
            return False
