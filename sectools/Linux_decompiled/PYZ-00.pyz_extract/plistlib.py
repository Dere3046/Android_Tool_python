...
PlistFormat = enum.Enum('PlistFormat', 'FMT_XML FMT_BINARY', __name__, **('module',))
...
class _PlistWriter(_DumbXMLWriter):
    ...
    def write_value(self, value):
        ...
        if None is False:
            ...
            return None
        if None(value, int):
            if value <= value or value < 0x10000000000000000L:
                pass
            else:
                -0x8000000000000000L
                raise OverflowError(value)
            -0x8000000000000000L.simple_element('integer', '%d' % value)
            return None
        raise None(value)
...
int.from_bytes(data[i:i + size], 'big'))(range(0, size * n, size)))
    ...
    def _read_object(self, ref):
        ...
        if token == 0:
            ...
            result = int.from_bytes(self._fp.read(1 << tokenL), 'big', tokenL >= 3, **('signed',))
        elif token == 34:
            ...
            result = datetime.datetime(2001, 1, 1) + datetime.timedelta(f, **('seconds',))
        elif tokenH == 64:
        ...
        if tokenH == 80:
            ...
            None((lambda .0 = None: for x in .0:
self._read_object(x))(obj_refs))
    # WARNING: Decompyle incomplete
...
_FORMATS = {
    FMT_BINARY: dict(_is_fmt_binary, _BinaryPlistParser, _BinaryPlistWriter, **('detect', 'parser', 'writer')),
    FMT_XML: dict(_is_fmt_xml, _PlistParser, _PlistWriter, **('detect', 'parser', 'writer')) }
...
def loads(value = None, *, fmt, dict_type):
    ...
    '''
    fp = BytesIO(value)
    return load(fp, fmt, dict_type, **('fmt', 'dict_type'))
...
def dump(value = None, fp = {
    ...
    writer = None[fmt]['writer'](fp, sort_keys, skipkeys, **('sort_keys', 'skipkeys'))
    writer.write(value)
...
def dumps(value = None, *, fmt, skipkeys, sort_keys):
    ...
    '''
    fp = BytesIO()
    dump(value, fp, fmt, skipkeys, sort_keys, **('fmt', 'skipkeys', 'sort_keys'))
    return fp.getvalue()
