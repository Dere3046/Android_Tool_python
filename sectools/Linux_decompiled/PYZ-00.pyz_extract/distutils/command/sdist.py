...
class sdist(Command):
    ...
    def _cs_path_exists(fspath):
        ...
        if not os.path.exists(fspath):
            return False
        abspath = None.path.abspath(fspath)
        (directory, filename) = os.path.split(abspath)
        return filename in os.listdir(directory)

    _cs_path_exists = staticmethod(_cs_path_exists)
    ...
    def read_template(self):
        ...
        template = TextFile(self.template, 1, 1, 1, 1, 1, 1, **('strip_comments', 'skip_blanks', 'join_lines', 'lstrip_ws', 'rstrip_ws', 'collapse_join'))
    ...
    def prune_file_list(self):
        ...
        self.filelist.exclude_pattern(None, build.build_base, **('prefix',))
        self.filelist.exclude_pattern(None, base_dir, **('prefix',))
        ...
        self.filelist.exclude_pattern(vcs_ptrn, 1, **('is_regex',))
    ...
    def make_release_tree(self, base_dir, files):
        ...
        dir_util.create_tree(base_dir, files, self.dry_run, **('dry_run',))
        if hasattr(os, 'link'):
            link = 'hard'
    ...
    def make_distribution(self):
        ...
        for fmt in self.formats:
            file = self.make_archive(base_name, fmt, base_dir, self.owner, self.group, **('base_dir', 'owner', 'group'))
            archive_files.append(file)
            self.distribution.dist_files.append(('sdist', '', file))
        self.archive_files = archive_files
        if not self.keep_temp:
            dir_util.remove_tree(base_dir, self.dry_run, **('dry_run',))
            return None
