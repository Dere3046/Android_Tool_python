...
class FancyGetopt:
    ...
    def add_option(self, long_option, short_option, help_string = (None, None)):
        ...
        option = (None, short_option, help_string)
    ...
    def _grok_option_table(self):
        ...
        for option in self.option_table:
            ...
            if None(long, str) or len(long) < 2:
            ...
            if not None is None:
                ...
                if long[-1] == '=':
                    ...
                    if alias_to is not None:
                        ...
                        self.long_opts[-1] = None
                    self.takes_arg[long] = 0
            alias_to = self.alias.get(long)
            if alias_to is not None and self.takes_arg[long] != self.takes_arg[alias_to]:
...
WS_TRANS = (lambda .0: pass# WARNING: Decompyle incomplete
)(string.whitespace)
...
def wrap_text(text, width):
    ...
    if None(text) <= width:
        return [
            text]
    text = None.expandtabs()
    text = text.translate(WS_TRANS)
    chunks = re.split('( +|-+)', text)
    chunks = (lambda .0: [ ch for ch in .0 if ch ])(chunks)
    lines = []
    if chunks:
        cur_line = []
        cur_len = 0
        if chunks:
            ...
            if cur_len + l <= width:
                ...
                del chunks[0]
                cur_len = cur_len + l
            elif cur_line and cur_line[-1][0] == ' ':
                del cur_line[-1]
        elif chunks or chunks:
            if cur_len == 0:
