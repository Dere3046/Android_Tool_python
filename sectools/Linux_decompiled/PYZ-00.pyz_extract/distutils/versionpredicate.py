...
class VersionPredicate:
    ...
    def __init__(self, versionPredicateStr):
        ...
        match = None.match(versionPredicateStr)
        if not match:
            raise ValueError('bad package name in %r' % versionPredicateStr)
        (self.name, paren) = None.groups()
        paren = paren.strip()
        if paren:
            ...
            str = None.groups()[0]
            self.pred = (lambda .0: [ splitUp(aPred) for aPred in .0 ])(str.split(','))
            if not self.pred:
    ...
    def __str__(self):
        if self.pred:
            seq = (lambda .0: [ cond + ' ' + str(ver) for cond, ver in .0 ])(self.pred)
