...
class CCompiler:
    ...
    def set_executables(self, **kwargs):
        ...
        for key in kwargs:
            ...
            None.set_executable(key, kwargs[key])
    ...
    def _check_macro_definitions(self, definitions):
        ...
        for defn in definitions:
            if isinstance(defn, tuple) and len(defn) in (1, 2):
                if not isinstance(defn[1], str) or defn[1] is None or isinstance(defn[0], str):
    ...
    def _setup_compile(self, outdir, macros, incdirs, sources, depends, extra):
        ...
        if None is None:
            extra = []
        objects = self.object_filenames(sources, 0, outdir, **('strip_dir', 'output_dir'))
    ...
    def _fix_compile_args(self, output_dir, macros, include_dirs):
        ...
        if None is None:
            ...
            raise TypeError("'include_dirs' (if supplied) must be a list of strings")
        return (None, macros, include_dirs)
    ...
    def _fix_lib_args(self, libraries, library_dirs, runtime_library_dirs):
        ...
        return (None, library_dirs, runtime_library_dirs)
    ...
    def _need_link(self, objects, output_file):
        ...
        if self.force:
            return True
        if None.dry_run:
            newer = newer_group(objects, output_file, 'newer', **('missing',))
            return newer
    ...
    def link_shared_lib(self, objects, output_libname, output_dir, libraries, library_dirs, runtime_library_dirs, export_symbols, debug, extra_preargs, extra_postargs, build_temp, target_lang = (None, None, None, None, None, 0, None, None, None, None)):
        self.link(CCompiler.SHARED_LIBRARY, objects, self.library_filename(output_libname, 'shared', **('lib_type',)), output_dir, libraries, library_dirs, runtime_library_dirs, export_symbols, debug, extra_preargs, extra_postargs, build_temp, target_lang)
    ...
    def has_function(self, funcname, includes, include_dirs, libraries, library_dirs = (None, None, None, None)):
        ...
        if library_dirs is None:
            library_dirs = []
        (fd, fname) = tempfile.mkstemp('.c', funcname, True, **('text',))
        f = os.fdopen(fd, 'w')
    # WARNING: Decompyle incomplete
    ...
    def spawn(self, cmd):
        spawn(cmd, self.dry_run, **('dry_run',))

    
    def move_file(self, src, dst):
        return move_file(src, dst, self.dry_run, **('dry_run',))

    
    def mkpath(self, name, mode = (511,)):
        mkpath(name, mode, self.dry_run, **('dry_run',))
...
def gen_preprocess_options(macros, include_dirs):
    ...
    for macro in macros:
        if isinstance(macro, tuple):
            if not len(macro) <= len(macro) or len(macro) <= 2:
                pass
            else:
                1
