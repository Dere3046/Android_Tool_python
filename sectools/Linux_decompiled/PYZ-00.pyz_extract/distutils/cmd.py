...
class Command:
    ...
    def dump_options(self, header, indent = (None, '')):
        ...
        self.announce(indent + header, log.INFO, **('level',))
        indent = indent + '  '
        for option, _, _ in self.user_options:
            ...
            self.announce(indent + '%s = %s' % (option, value), log.INFO, **('level',))
...
isinstance(v, str))(val))
    ...
    def _ensure_tested_string(self, option, tester, what, error_fmt, default = (None,)):
        ...
        if not val is not None or tester(val):
    ...
    def execute(self, func, args, msg, level = (None, 1)):
        util.execute(func, args, msg, self.dry_run, **('dry_run',))

    
    def mkpath(self, name, mode = (511,)):
        dir_util.mkpath(name, mode, self.dry_run, **('dry_run',))
    ...
    def copy_file(self, infile, outfile, preserve_mode, preserve_times, link, level = (1, 1, None, 1)):
        ...
        return file_util.copy_file(infile, outfile, preserve_mode, preserve_times, not (self.force), link, self.dry_run, **('dry_run',))
    ...
    def copy_tree(self, infile, outfile, preserve_mode, preserve_times, preserve_symlinks, level = (1, 1, 0, 1)):
        ...
        and force flags.
        '''
        return dir_util.copy_tree(infile, outfile, preserve_mode, preserve_times, preserve_symlinks, not (self.force), self.dry_run, **('dry_run',))

    
    def move_file(self, src, dst, level = (1,)):
        '''Move a file respecting dry-run flag.'''
        return file_util.move_file(src, dst, self.dry_run, **('dry_run',))
    ...
    def spawn(self, cmd, search_path, level = (1, 1)):
        ...
        spawn(cmd, search_path, self.dry_run, **('dry_run',))

    
    def make_archive(self, base_name, format, root_dir, base_dir, owner, group = (None, None, None, None)):
        return archive_util.make_archive(base_name, format, root_dir, base_dir, self.dry_run, owner, group, **('dry_run', 'owner', 'group'))
