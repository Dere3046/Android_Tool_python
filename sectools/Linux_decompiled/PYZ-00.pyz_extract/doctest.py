...
def _normalize_module(module, depth = (2,)):
    ...
    raise None('Expected a module, string, or None')
...
def _exception_traceback(exc_info):
    ...
    traceback.print_exception(exc_type, exc_val, exc_tb, excout, **('file',))
...
class _OutputRedirectingPdb(pdb.Pdb):
    ...
    def __init__(self, out):
        ...
        pdb.Pdb.__init__(self, out, True, **('stdout', 'nosigint'))
        self.use_rawinput = 1
...
class Example:
    ...
    def __init__(self, source, want, exc_msg, lineno, indent, options = (None, 0, 0, None)):
        ...
        if not want and want.endswith('\n'):
            want += '\n'
        if not exc_msg is not None and exc_msg.endswith('\n'):
            exc_msg += '\n'
        self.source = source
        self.want = want
        self.lineno = lineno
    ...
    def __eq__(self, other):
        ...
        if None.source == other.source and self.want == other.want and self.lineno == other.lineno and self.indent == other.indent and self.options == other.options:
            pass
...
class DocTest:
    ...
    def __eq__(self, other):
        if type(self) is not type(other):
            return NotImplemented
        if None.examples == other.examples and self.docstring == other.docstring and self.globs == other.globs and self.name == other.name and self.filename == other.filename:
            pass
...
class DocTestParser:
    ...
    def parse(self, string, name = ('<string>',)):
        ...
        if min_indent > 0:
            string = None((lambda .0 = None: [ l[min_indent:] for l in .0 ])(string.split('\n')))
        output = []
        (charno, lineno) = (0, 0)
        for m in self._EXAMPLE_RE.finditer(string):
            ...
            if not self._IS_BLANK_OR_COMMENT(source):
                output.append(Example(source, want, exc_msg, lineno, min_indent + len(m.group('indent')), options, **('lineno', 'indent', 'options')))
            lineno += string.count('\n', m.start(), m.end())
            charno = m.end()
    ...
    def get_examples(self, string, name = ('<string>',)):
        ...
        '''
        return (lambda .0: [ x for x in .0 if isinstance(x, Example) ])(self.parse(string, name))
    ...
    def _parse_example(self, m, name, lineno):
        ...
        source = None((lambda .0 = None: [ sl[indent + 4:] for sl in .0 ])(source_lines))
        want = m.group('want')
        want_lines = want.split('\n')
        if len(want_lines) > 1 and re.match(' *$', want_lines[-1]):
            del want_lines[-1]
        self._check_prefix(want_lines, ' ' * indent, name, lineno + len(source_lines))
        want = None((lambda .0 = None: [ wl[indent:] for wl in .0 ])(want_lines))
    ...
    def _min_indent(self, s):
        ...
        indents = (lambda .0: [ len(indent) for indent in .0 ])(self._INDENT_RE.findall(s))
        if len(indents) > 0:
            return min(indents)
    ...
    def _check_prefix(self, lines, prefix, name, lineno):
        ...
        '''
        for i, line in enumerate(lines):
            if not line and line.startswith(prefix):
...
class DocTestFinder:
    ...
    def _from_module(self, module, object):
        ...
        if module is None:
            return True
        if None.getmodule(object) is not None:
            return module is inspect.getmodule(object)
        if None.isfunction(object):
            return module.__dict__ is object.__globals__
        if None.ismethoddescriptor(object):
            if hasattr(object, '__objclass__'):
                ...
                return True
            return None.__name__ == obj_mod
        if None.isclass(object):
            return module.__name__ == object.__module__
        if None(object, '__module__'):
            return module.__name__ == object.__module__
        if None(object, property):
            return True
        raise None('object must be a class or function')
    ...
    def _find(self, tests, obj, name, module, source_lines, globs, seen):
        ...
        if inspect.ismodule(obj) and self._recurse:
            for valname, val in getattr(obj, '__test__', { }).items():
                ...
                if not None.isroutine(val) and inspect.isclass(val) and inspect.ismodule(val) and isinstance(val, str):
                    raise ValueError('DocTestFinder.find: __test__ values must be strings, functions, methods, classes, or modules: %r' % (type(val),))
                valname = None % (name, valname)
                self._find(tests, val, valname, module, source_lines, globs, seen)
        if inspect.isclass(obj) or self._recurse:
            for valname, val in obj.__dict__.items():
                if isinstance(val, (staticmethod, classmethod)):
                    val = val.__func__
                if (inspect.isroutine(val) and inspect.isclass(val) or isinstance(val, property)) and self._from_module(module, val):
                    valname = '%s.%s' % (name, valname)
                    self._find(tests, val, valname, module, source_lines, globs, seen)
            return None
        return None
    ...
    def _find_lineno(self, obj, source_lines):
        ...
        if inspect.isclass(obj):
            if source_lines is None:
                return None
            pat = None.compile('^\\s*class\\s*%s\\b' % getattr(obj, '__name__', '-'))
...
class DocTestRunner:
    ...
    def __patched_linecache_getlines(self, filename, module_globals = (None,)):
        ...
        if m and m.group('name') == self.test.name:
            ...
            return example.source.splitlines(True, **('keepends',))
...
class OutputChecker:
    ...
    def check_output(self, want, got, optionflags):
        ...
        if got == want:
            return True
        if not None & DONT_ACCEPT_TRUE_FOR_1:
            if (got, want) == ('True\n', '1\n'):
                return True
            if (None, want) == ('False\n', '0\n'):
                return True
            if not None & DONT_ACCEPT_BLANKLINE:
                want = re.sub('(?m)^%s\\s*?$' % re.escape(BLANKLINE_MARKER), '', want)
                got = re.sub('(?m)^[^\\S\\n]+$', '', got)
                if got == want:
                    return True
                if None & NORMALIZE_WHITESPACE:
                    got = ' '.join(got.split())
                    want = ' '.join(want.split())
                    if got == want:
                        return True
                    if None & ELLIPSIS and _ellipsis_match(want, got):
                        return True
                    return None
...
def testmod(m, name, globs, verbose, report, optionflags, extraglobs, raise_on_error, exclude_empty = (None, None, None, None, True, 0, None, False, False)):
    ...
    if raise_on_error:
        runner = DebugRunner(verbose, optionflags, **('verbose', 'optionflags'))
    else:
        runner = DocTestRunner(verbose, optionflags, **('verbose', 'optionflags'))
    for test in finder.find(m, name, globs, extraglobs, **('globs', 'extraglobs')):
        runner.run(test)
    if report:
        runner.summarize()
    if master is None:
...
def testfile(filename, module_relative, name, package, globs, verbose, report, optionflags, extraglobs, raise_on_error, parser, encoding = (True, None, None, None, None, True, 0, None, False, DocTestParser(), None)):
    ...
    if not encoding:
        pass
    (text, filename) = None(filename, package, module_relative, 'utf-8')
    if name is None:
...
def run_docstring_examples(f, globs, verbose, name, compileflags, optionflags = (False, 'NoName', None, 0)):
    ...
    information.
    """
    finder = DocTestFinder(verbose, False, **('verbose', 'recurse'))
    runner = DocTestRunner(verbose, optionflags, **('verbose', 'optionflags'))
...
class DocTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        runner = DocTestRunner(optionflags, self._dt_checker, False, **('optionflags', 'checker', 'verbose'))
    # WARNING: Decompyle incomplete
    ...
    def debug(self):
        """Run the test case without results and without catching exceptions
           ...
           """
        self.setUp()
        runner = DebugRunner(self._dt_optionflags, self._dt_checker, False, **('optionflags', 'checker', 'verbose'))
        runner.run(self._dt_test, False, **('clear_globs',))
        self.tearDown()
    ...
    def __eq__(self, other):
        if type(self) is not type(other):
            return NotImplemented
        if None._dt_test == other._dt_test and self._dt_optionflags == other._dt_optionflags and self._dt_setUp == other._dt_setUp and self._dt_tearDown == other._dt_tearDown:
            pass
...
def DocTestSuite(module, globs, extraglobs, test_finder = (None, None, None, None), **options):
    ...
    tests = test_finder.find(module, globs, extraglobs, **('globs', 'extraglobs'))
    if tests and sys.flags.optimize >= 2:
        suite = _DocTestSuite()
...
def DocFileTest(path, module_relative, package, globs, parser, encoding = (True, None, None, DocTestParser(), None), **options):
    ...
    if not encoding:
        pass
    (doc, path) = None(path, package, module_relative, 'utf-8')
    ...
    for piece in DocTestParser().parse(s):
        if isinstance(piece, Example):
            ...
            if want:
                ...
                output += (lambda .0: [ '## ' + l for l in .0 ])(want.split('\n')[:-1])
            continue
        output += (lambda .0: [ _comment_line(l) for l in .0 ])(piece.split('\n')[:-1])
    if output and output[-1] == '#':
        output.pop()
        if output:
            if (output[-1] == '#' or output) and output[0] == '#':
                output.pop(0)
                if output:
                    if not output[0] == '#':
                        return '\n'.join(output) + '\n'
...
def testsource(module, name):
    ...
    test = (lambda .0 = None: [ t for t in .0 if t.name == name ])(tests)
    if not test:
...
def debug_script(src, pm, globs = (False, None)):
    ...
    if pm:
        ...
        try:
            ...
            p = pdb.Pdb(True, **('nosigint',))
            p.reset()
            p.interaction(None, sys.exc_info()[2])
            return None
            pdb.Pdb(True, **('nosigint',)).run('exec(%r)' % src, globs, globs)
            return None
...
def _test():
    import argparse
    parser = argparse.ArgumentParser('doctest runner', **('description',))
    parser.add_argument('-v', '--verbose', 'store_true', False, 'print very verbose output for all tests', **('action', 'default', 'help'))
    parser.add_argument('-o', '--option', 'append', OPTIONFLAGS_BY_NAME.keys(), [], 'specify a doctest option flag to apply to the test run; may be specified more than once to apply multiple options', **('action', 'choices', 'default', 'help'))
    parser.add_argument('-f', '--fail-fast', 'store_true', 'stop running tests after first failure (this is a shorthand for -o FAIL_FAST, and is in addition to any other -o options)', **('action', 'help'))
    parser.add_argument('file', '+', 'file containing the tests to run', **('nargs', 'help'))
    ...
    for filename in testfiles:
        if filename.endswith('.py'):
            ...
            del sys.path[0]
            (failures, _) = testmod(m, verbose, options, **('verbose', 'optionflags'))
        else:
            (failures, _) = testfile(filename, False, verbose, options, **('module_relative', 'verbose', 'optionflags'))
        if failures:
            return 1
        return 0
