...
class TestResult(object):
    ...
    def wasSuccessful(self):
        ...
        if len(self.errors) == len(self.errors):
            pass
        elif not len(self.errors) == 0 and not hasattr(self, 'unexpectedSuccesses'):
            pass
    ...
    def _exc_info_to_string(self, err, test):
        ...
        if tb and self._is_relevant_tb_level(tb):
            tb = tb.tb_next
            if tb:
                if self._is_relevant_tb_level(tb) or exctype is test.failureException:
                    length = self._count_relevant_tb_levels(tb)
                else:
                    length = None
        tb_e = traceback.TracebackException(exctype, value, tb, length, self.tb_locals, True, **('limit', 'capture_locals', 'compact'))
