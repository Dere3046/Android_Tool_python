...
class Node(object):
    ...
    def show(self, buf, offset, attrnames, nodenames, showcoord, _my_node_name = (sys.stdout, 0, False, False, False, None)):
        ...
        if self.attr_names:
            if attrnames:
                nvlist = (lambda .0 = None: [ (n, getattr(self, n)) for n in .0 ])(self.attr_names)
                attrstr = ', '.join((lambda .0: for nv in .0:
'%s=%s' % nv)(nvlist))
            else:
                vlist = (lambda .0 = None: [ getattr(self, n) for n in .0 ])(self.attr_names)
                attrstr = ', '.join((lambda .0: for v in .0:
'%s' % v)(vlist))
            buf.write(attrstr)
        if showcoord:
        ...
        for child_name, child in self.children():
            child.show(buf, offset + 2, attrnames, nodenames, showcoord, child_name, **('offset', 'attrnames', 'nodenames', 'showcoord', '_my_node_name'))
