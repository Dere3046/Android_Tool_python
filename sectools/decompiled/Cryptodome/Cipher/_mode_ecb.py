...
class EcbMode(object):
    ...
    def __init__(self, block_cipher):
        ...
        self._state = None(self._state.get(), raw_ecb_lib.ECB_stop_operation)
    ...
    def encrypt(self, plaintext, output = (None,)):
        ...
        if output is None:
            ...
            result = None.ECB_encrypt(self._state.get(), c_uint8_ptr(plaintext), c_uint8_ptr(ciphertext), c_size_t(len(plaintext)))
            if result:
                if result == 3:
                    raise ValueError('Data must be aligned to block boundary in ECB mode')
                raise None('Error %d while encrypting in ECB mode' % result)
            if None is None:
    ...
    def decrypt(self, ciphertext, output = (None,)):
        ...
        if output is None:
            ...
            if None(ciphertext) != len(output):
                raise ValueError('output must have the same length as the input  (%d bytes)' % len(plaintext))
            result = None.ECB_decrypt(self._state.get(), c_uint8_ptr(ciphertext), c_uint8_ptr(plaintext), c_size_t(len(ciphertext)))
            if result:
                if result == 3:
                    raise ValueError('Data must be aligned to block boundary in ECB mode')
                raise None('Error %d while decrypting in ECB mode' % result)
            if None is None:
